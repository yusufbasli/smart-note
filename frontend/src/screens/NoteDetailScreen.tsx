...
export default function NoteDetailScreen({ navigation, route }: NotesScreenProps<"NoteDetail">) {
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={{ padding: 16, paddingBottom: 40 }}
      refreshControl={
        <RefreshControl refreshing={isLoading} onRefresh={() => fetchNote(noteId)} />
      }
    >
      {/* Title + category */}
      <View className="flex-row justify-between items-start mb-2">
        ...
        {currentNote.ai_category && (
          <CategoryBadge category={currentNote.ai_category} />
        )}
      </View>
      <Text className="text-xs text-gray-400 mb-4">
        Updated {new Date(currentNote.updated_at).toLocaleString()}
      </Text>

      {/* AI Summary */}
      {currentNote.ai_summary && (
        <View className="bg-primary-50 border border-primary-100 rounded-xl p-4 mb-4">
          <Text className="text-xs font-semibold text-primary-600 mb-1">AI SUMMARY</Text>
          <Text className="text-sm text-gray-700">{currentNote.ai_summary}</Text>
        </View>
      )}
      ...
      <View className="gap-3">
        ...
        >
          <Text className="text-white font-semibold">Edit Note</Text>
        </TouchableOpacity>

        <TouchableOpacity
          className="bg-white border border-primary-600 rounded-xl py-3 items-center flex-row justify-center gap-2"
        ...
        >
          {analyzing ? (
            ...
            <Text className="text-primary-600 font-semibold">✨ Re-analyze with AI</Text>
          )}
        </TouchableOpacity>

        <TouchableOpacity
          className="bg-white border border-red-300 rounded-xl py-3 items-center"
          onPress={handleDelete}
        >
          <Text className="text-red-500 font-semibold">Delete Note</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
